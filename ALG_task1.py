...
def check_2(lst_obj):
    ...
    for j in range(len(lst_obj)):          # O(N)
        if lst_obj[j] in lst_obj[j+1:]:    #  получение индекса + сложение + создание среза + проверка in: O(1) + O(1) + O(N) + O(N) = O(N)
            return False                   # O(1)
    return True                            # O(1)
...
def check_3(lst_obj):
    ...
    for i in range(len(lst_obj) - 1):        # O(N)
        if lst_copy[i] == lst_copy[i+1]:     # O(1)
            return False                     # O(1)
    return True                              # O(1)
    # O(N)+O(N*logN)+O(N)*O(1)+O(1)= O(N+N*logN+N+1)=O(2N+N*logN+1)=O(N*logN)
#############################################################################################
...
for j in (50, 500, 1000, 5000, 1000):  #O(1)
    ...
    # Всего 10 тыс. чисел
    lst = random.sample(range(-100000, 100000), j) #O(len(N))


print(check_1(lst)) #O(1)
print(check_2(lst)) #O(1)
print(check_3(lst)) #O(1)
 #O(len(N))*O(1)+3*O(1) = O(1)